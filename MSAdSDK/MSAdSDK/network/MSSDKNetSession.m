...
@implementation MSSDKNetSession
  static  id _instance;
    ...
    WS(ws);
        if (maskType !=WsqflyNetSessionMaskStateNone) {    // 有菊花动画
                dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        [[UIApplication sharedApplication].keyWindow addSubview:ws.wsqAView];
            
                        ws.wsqAView.hidden =NO;
            
                        [ws.wsqAView startAnimating];
            
                        ws.wsqAView.userInteractionEnabled = (maskType ==WsqflyNetSessionMaskStateNotTouch ?YES : NO);
                        ws.maskCount++;
                    });
        ...
        if (maskType !=WsqflyNetSessionMaskStateNone) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        if (ws.maskCount <=0) {
                                ...
                                [ws.wsqAView stopAnimating];
                
                                ws.wsqAView.hidden =YES;
                                [ws.wsqAView removeFromSuperview];
                            }
